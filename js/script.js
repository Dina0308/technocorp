...
$(".wins__slider__video video").click(function () {
	let video = $(this)[0]; // выбираем конкретное видео
	if (video.paused) {
	  video.play();
	  $(this).siblings('.play-video').fadeOut(); // выбираем только кнопку внутри текущего блока
	} else {
	  video.pause();
	  $(this).siblings('.play-video').fadeIn(); // выбираем только кнопку внутри текущего блока
	}
  });
...
//отображать корректно слайды в кейсе, если их 2 и меньше

function slides() {
	let slideCount1 = $('.tabs__content.active .cases__slider-nav').find('.cases__slide').length;
	console.log(slideCount1);
	if (slideCount1 <=1) {
		$('.tabs__content.active .cases__slider-nav').find('.slick-track').css('width','100%');
		$('.tabs__content.active .open-gallery').css('left','258px');
	}
	if (slideCount1 == 2) {
		
		$('.tabs__content.active .open-gallery').css('left','300px');
	}
}
function slidesMob() {
	let slideCount1 = $('.tabs__content.active .cases__slider-nav').find('.cases__slide').length;
	...
	if (slideCount1 <=1) {
		$('.tabs__content.active .cases__slider-nav').find('.slick-track').css('width','100%');
		$('.tabs__content.active .open-gallery').css('left','44%');
	}
	if (slideCount1 == 2) {
		
		$('.tabs__content.active .open-gallery').css('left','49%');
	}
}
...
$('.cases .tabs__caption li').each(function(i){
	$(this).on('click', function() {
		...
		if( window.innerWidth >= 768 ){
			let slideCount = $('.cases__slider-nav').eq(i).find('.cases__slide').length;
			console.log(slideCount);
			if (slideCount <=1) {
				$('.cases__slider-nav').eq(i).find('.slick-track').css('width','100%');
				$('.open-gallery').eq(i).css('left','258px');
			}
			if (slideCount == 2) {
				
				$('.open-gallery').eq(i).css('left','300px');
			}
		} else {
			let slideCount = $('.cases__slider-nav').eq(i).find('.cases__slide').length;
			console.log(slideCount);
			if (slideCount <=1) {
				$('.cases__slider-nav').eq(i).find('.slick-track').css('width','100%');
				$('.open-gallery').eq(i).css('left','44%');
			}
			if (slideCount == 2) {
				
				$('.open-gallery').eq(i).css('left','49%');
			}
		} 
	});
});
